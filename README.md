...
Вкладка **«Автопайплайн»** обрабатывает сразу набор экспериментов из корневой папки с файлами
`experiment*.afxml`:

1. Выберите папку замеров и папку результата.
2. Нажмите **«Сканировать эксперименты»** и снимите отметки с ненужных экспериментов.
3. Задайте параметры сортировки, PTV, фильтрации, усреднения и преобразования координат.
4. Выберите гистограммы и векторные графики, а также форматы экспорта PNG и/или CSV.
5. Запустите пайплайн. Общий прогресс, текущий этап и результат каждого эксперимента отображаются справа.

В корне результата сохраняются `pipeline_config.json` с полным набором параметров и
`pipeline_report.csv` со статусом каждого эксперимента. Графики отдельного эксперимента
сохраняются в папке `PTV_.../graphs`.
