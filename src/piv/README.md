...
PIV анализ вычисляет векторное поле скоростей между парами кадров, используя метод кросс-корреляции. Модуль включает:

- Анализ пар изображений с суффиксами _a и _b (например: 1_a.png, 1_b.png)
...
Модуль ожидает пары изображений с суффиксами `_a` и `_b`:
- `100_a.png` и `100_b.png` - первая пара (интервал 2 мкс)
- `101_a.png` и `101_b.png` - вторая пара (интервал 2 мкс)
- И так далее...

Между парами интервал составляет ~1 секунду, внутри пары - 2 мкс (0.002 мс).
...
- **dt** - временной интервал между кадрами в паре (_a и _b) в мс
  - По умолчанию: 0.002 мс (2 мкс)
  - Используется для вычисления скорости из смещения
  - Между парами (1, 2, 3...) интервал ~1 секунда, но каждая пара обрабатывается независимо
